...
class KeyboardTeleopNode(Node):
    def __init__(self):
        ...
        self.declare_parameter('motion_hold_sec', 0.35)
        ...
        self._motion_hold_sec = max(float(self.get_parameter('motion_hold_sec').value), 0.05)
        ...
        self._key_bindings = build_key_bindings(self._linear_step_x, self._linear_step_z)
    ...
    def _run_scripted_actions(self, now_sec: float) -> None:
        ...
        action = parse_script_action(action_text, self._linear_step_x, self._linear_step_z)
    ...
    def _apply_action(self, action) -> None:
        ...
        if action.kind == 'velocity':
            ...
            self._last_motion_command_sec = now_sec
            self._io.publish_velocity(self._current_linear_x, self._current_linear_z)
            self._last_publish_sec = now_sec
            self.get_logger().info(
                f'Published motion command {action.label}: '
                f'linear_x={self._current_linear_x:.2f} linear_z={self._current_linear_z:.2f}'
            )
    ...
    def _publish_motion(self, now_sec: float) -> None:
        ...
        if (now_sec - self._last_motion_command_sec) > self._motion_hold_sec:
            if self._current_linear_x != 0.0 or self._current_linear_z != 0.0:
                self._current_linear_x = 0.0
                self._current_linear_z = 0.0
                self._io.publish_zero()
                self._last_publish_sec = now_sec
    ...
    def _print_status(self, now_sec: float) -> None:
        ...
        self.get_logger().info(self._runtime_state.status_line(now_sec, self._telemetry_timeout_sec))

...
class SwarmLatencyRecorder(Node):
    def __init__(self, manifest_path: Path, output_dir: Path):
        super().__init__('swarm_latency_recorder')
        ...
        for drone in manifest.get('drones', []):
            ...
            namespace = str(drone['namespace']).rstrip('/')
            self._subscribe(Twist, f'{namespace}/alate_input_velocity', drone_id, 'input_velocity', self._on_velocity)
            self._subscribe(OpCom, f'{namespace}/alate_input_operator_command', drone_id, 'input_operator_command', self._on_opcom)
            self._subscribe(McState, f'{namespace}/alate_output_mission_control_state', drone_id, 'mission_control_state', self._on_mc_state, state_qos)
            self._subscribe(HlcState, f'{namespace}/alate_output_high_level_control_state', drone_id, 'high_level_control_state', self._on_hlc_state, state_qos)
            self._subscribe(HlcTelemetry, f'{namespace}/alate_output_high_level_control_telemetry', drone_id, 'hlc_telemetry', self._on_telemetry)
            self._subscribe(
                HlcPlatformError,
        ...
        self.get_logger().info(f'Swarm latency recorder started for {len(manifest.get("drones", []))} drones')
    ...
    def _emit_summary(self) -> None:
        ...
        self.get_logger().info(f'swarm_latency_recorder captured {self._event_count} events ({rate:.2f} events/sec)')
        self._event_count = 0
        self._last_summary_sec = now


def parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description='Record namespaced ROS latency events for the swarm stack.')
...
def main(argv: list[str] | None = None) -> int:
    ...
    rclpy.init()
    node = SwarmLatencyRecorder(args.manifest, args.output_dir)
